...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  ENABLE_FALLBACK     Fall back to a local model if the provider fails"
            echo "                      (rate limit, timeout, downtime). Default: true"
            echo "  FALLBACK_PROVIDER   Provider to fall back to. Default: ollama"
            echo "  GH_COMMIT_AI_ERROR_LOG  Provider-failure log (daily-rotated)"
            echo "                      Default: /tmp/gh-commit-ai-errors.log"
            ...
            echo "                      Default: true"
            echo "  DIFF_MAX_LINES      Max diff lines to send to AI (default: 200)"
            echo "  GH_COMMIT_AI        Set to 1 to enable hook (opt-in)"
            echo ""
            echo "Two-Model Pipeline:"
            echo "  PIPELINE_ENABLED    Enable two-stage pipeline (default: false)"
            echo "  ANALYSIS_MODEL      Ollama model for Stage 1 analysis"
            echo "  ANALYSIS_PROVIDER   Provider for Stage 1 (defaults to AI_PROVIDER)"
            echo "  Stage 2 uses default model (OLLAMA_MODEL, etc.)"
            echo ""
