call_groq() {
    ...
    if should_stream; then
        ...
        if streaming_api_call \
            ...
            -H "Authorization: Bearer $GROQ_API_KEY"; then
    ...
    if [ "$exit_code" != "0" ] || [ -z "$response" ]; then
        ...
        echo "  • Groq API is experiencing downtime" >&2
        echo "  • Request timeout (large diff or slow connection)" >&2
        echo "  • Rate limiting or quota exceeded" >&2
        echo "" >&2
        echo "Suggestions:" >&2
        echo "  • Check your internet: ping api.groq.com" >&2
        echo "  • Verify service status: https://status.groq.com/" >&2
